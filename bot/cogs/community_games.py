...
class CommunityGames(commands.Cog):
    ...
    async def open_registration_command(self, ctx):
        if self.client.globale_variables.registration_opened:
        ...
        del self.client.globale_variables.players_list[:]
        ...
        self.client.globale_variables.registration_opened = True
    ...
    async def close_registration_command(self, ctx):
        if not self.client.globale_variables.registration_opened:
        ...
        self.client.globale_variables.registration_opened = False
    ...
    async def add_participant_command(self, ctx):
        ...
        if player_name in self.client.globale_variables.bench or player_name in self.client.globale_variables.players_list:
        ...
        if len(self.client.globale_variables.bench) > 0:
            ...
            self.client.globale_variables.bench.append(player_name)

        log.debug('Add player to [players_list]')
        self.client.globale_variables.players_list.append(player_name)
    ...
    async def remove_participant_command(self, ctx):
        ...
        if in_game_name in self.client.globale_variables.players_list:
            log.debug('Remove player from the players list')
            self.client.globale_variables.players_list.remove(in_game_name)
        if in_game_name in self.client.globale_variables.bench:
            log.debug('Remove player from the bench')
            self.client.globale_variables.bench.remove(in_game_name)
        if in_game_name in self.client.globale_variables.players_allowed_to_play:
            log.debug('Remove player from the allowed to play list')
            self.client.globale_variables.players_allowed_to_play.remove(in_game_name)
    ...
    async def show_participants_command(self, ctx):
        ...
        for x in range(0, len(self.client.globale_variables.players_list)):
            embed.add_field(name="Player " + str(x + 1) + ":", value=self.client.globale_variables.players_list[x], inline=True)
    ...
    async def add_user_command(self, ctx, user: discord.User):
        ...
        if in_game_name in self.client.globale_variables.bench or in_game_name in self.client.globale_variables.players_list:
        ...
        self.client.globale_variables.players_list.append(in_game_name)

        # If first teams were generated already, add to bench as well
        log.debug('Check if player are already on bench')
        if len(self.client.globale_variables.bench) > 0:
            log.debug('Add player to bench')
            self.client.globale_variables.bench.append(in_game_name)
