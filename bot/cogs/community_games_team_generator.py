...
class CommunityGamesTeamGenerator(commands.Cog):
    ...
    async def generate_teams_command(self, ctx):
        ...
        number_of_players = len(self.client.globale_variables.players_list)
        ...
        del self.client.globale_variables.already_used_index[:]
        log.debug('Delete globale variable [teams]')
        del self.client.globale_variables.teams[:]

        self.fill_players_allowed_to_play()

        number_of_players_allowed = len(self.client.globale_variables.players_allowed_to_play)
        ...
        self.generate_teams()
        await ctx.send(embed=self.generate_team_embed_message(1, self.client.globale_variables.teams[0]))
        await ctx.send(embed=self.generate_team_embed_message(2, self.client.globale_variables.teams[1]))

    def generate_team(self):
        log.debug('Start generating teams')
        number_of_players = len(self.client.globale_variables.players_allowed_to_play)
        team = list()
        x = 0
        while x < 6:
            player_index = randrange(number_of_players)
            if player_index in self.client.globale_variables.already_used_index:
                log.debug('Index already used')
                continue
            self.client.globale_variables.already_used_index.append(player_index)
            log.debug('Add player to the team: ' + self.client.globale_variables.players_allowed_to_play[player_index])
            team.append(self.client.globale_variables.players_allowed_to_play[player_index])
            x += 1
        return team

    def generate_teams(self):
        for x in range(0, 2):
            self.client.globale_variables.teams.append(self.generate_team())
    ...
    def fill_players_allowed_to_play(self):
        ...
        if len(self.client.globale_variables.bench) > 0:
            log.debug('Already benched players')
            # Get amount of player who have to get benched
            players_to_bench = len(self.client.globale_variables.bench)
            log.debug('Amount of player to bench: ' + str(players_to_bench))

            if len(self.client.globale_variables.players_allowed_to_play) < 12:
                log.debug('Less than 12 players')
                missing_player = 12 - len(self.client.globale_variables.players_allowed_to_play)
            ...
            while x < players_to_bench:
                player_index_to_remove = randint(0, len(self.client.globale_variables.players_allowed_to_play) - 1)
                ...
                player_name = self.client.globale_variables.players_allowed_to_play[player_index_to_remove]

                if player_name not in self.client.globale_variables.players_list:
                ...
                temp_bench.append(self.client.globale_variables.players_allowed_to_play[player_index_to_remove])
                x = x + 1

            # Empty list of benched players
            log.debug('Empty globale variable [bench]')
            del self.client.globale_variables.bench[:]
            ...
            del self.client.globale_variables.players_allowed_to_play[:]

            players_allowed_to_play_amount = len(self.client.globale_variables.players_list) - len(temp_bench)
            log.debug('Amount of players allowed to play: ' + str(players_allowed_to_play_amount))

            log.debug('Start filling allowed player')
            x = 0
            while x < players_allowed_to_play_amount:
                allowed_to_play_index = randint(0, len(self.client.globale_variables.players_list) - 1)
                log.debug('Try to add player index: ' + str(allowed_to_play_index))
                if allowed_to_play_index in temp_used_index:
                    log.debug('Index already used')
                    continue

                player_name = self.client.globale_variables.players_list[allowed_to_play_index]
                if player_name in self.client.globale_variables.bench:
                ...
                self.client.globale_variables.players_allowed_to_play.append(player_name)
                x = x + 1

            log.debug('Fill in benched with temp_bench')
            for benched in temp_bench:
                self.client.globale_variables.bench.append(benched)
            ...
            players_to_bench = len(self.client.globale_variables.players_list) - 12
            log.debug('Amount of player to bench: ' + str(players_to_bench))
            used_index = list()
            x = 0
            ...
            while x < players_to_bench:
                ...
                if player_to_remove_index in used_index:
                    log.debug('Index already used')
                    continue
                self.client.globale_variables.bench.append(self.client.globale_variables.players_list[player_to_remove_index])
                used_index.append(player_to_remove_index)
                x += 1
            log.debug('Start adding allowed player')
            for x in range(0, len(self.client.globale_variables.players_list)):
                if x in used_index:
                    log.debug('Index already used')
                    continue
                self.client.globale_variables.players_allowed_to_play.append(self.client.globale_variables.players_list[x])
