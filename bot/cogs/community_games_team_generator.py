...
class CommunityGamesTeamGenerator(commands.Cog):
    ...
    async def generate_teams_command(self, ctx):
        ...
        number_of_players_allowed = len(self.client.global_variables.players_allowed_to_play)
        ...
        self.generate_teams()
        await ctx.send(embed=self.generate_team_embed_message(1, self.client.global_variables.teams[0]))
        await ctx.send(embed=self.generate_team_embed_message(2, self.client.global_variables.teams[1]))
    ...
    def generate_team(self):
        ...
        while x < 6:
            ...
            log.debug('Add player to the team: ' + self.client.global_variables.players_allowed_to_play[player_index])
            team.append(self.client.global_variables.players_allowed_to_play[player_index])
            x += 1
        return team
    ...
    def fill_players_allowed_to_play(self):
        ...
        if len(self.client.global_variables.bench) > 0:
            ...
            if len(self.client.global_variables.players_allowed_to_play) < 12:
                ...
                missing_player = 12 - len(self.client.global_variables.players_allowed_to_play)
            ...
            while x < players_to_bench:
                player_index_to_remove = randint(0, len(self.client.global_variables.players_allowed_to_play) - 1)
                ...
                player_name = self.client.global_variables.players_allowed_to_play[player_index_to_remove]
                ...
                temp_bench.append(self.client.global_variables.players_allowed_to_play[player_index_to_remove])
                x = x + 1
            ...
            players_allowed_to_play_amount = len(self.client.global_variables.players_list) - len(temp_bench)
            ...
            while x < players_allowed_to_play_amount:
                allowed_to_play_index = randint(0, len(self.client.global_variables.players_list) - 1)
                log.debug('Try to add player index: ' + str(allowed_to_play_index))
                if allowed_to_play_index in temp_used_index:
                    log.debug('Index already used')
                    continue

                player_name = self.client.global_variables.players_list[allowed_to_play_index]
